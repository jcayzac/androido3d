...
inline bool IsWhitespace(char c) {
	...
	while(kWhitespaceASCII[i])
		if(c == kWhitespaceASCII[i]) return true;
...
bool StartsWithASCII(const std::string& str,
	...
	if(case_sensitive)
		...
		return ::o3d::base::strncasecmp(str.c_str(), search.c_str(), search.length()) == 0;
}
...
inline int vsnprintfT(char* buffer,
                      ...
                      const char* format,
                      va_list argptr) {
	return ::o3d::base::vsnprintf(buffer, buf_size, format, argptr);
}
...
static void StringAppendVT(StringType* dst,
	...
	va_copy(ap_copy, ap);
	errno = 0;
	int result = vsnprintfT(stack_buf, o3d_arraysize(stack_buf), format, ap_copy);
	va_end(ap_copy);

	if(result >= 0 && result < static_cast<int>(o3d_arraysize(stack_buf))) {
		// It fit.
	...
	int mem_length = o3d_arraysize(stack_buf);

	while(true) {
		if(result < 0) {
			if(errno != 0 && errno != EOVERFLOW) {
				// If an error other than overflow occurred, it's never going to work.
				O3D_LOG(WARNING) << "Unable to printf the requested string due to error.";
				return;
			}
		...
		if(mem_length > 32 * 1024 * 1024) {
			...
			O3D_LOG(WARNING) << "Unable to printf the requested string due to size.";
			return;
		}
	...
	for(typename FormatStringType::const_iterator i = format_string.begin();
		...
		if('$' == *i) {
			if(i + 1 != format_string.end()) {
				++i;
				O3D_ASSERT('$' == *i || '1' <= *i) << "Invalid placeholder: " << *i;
...
bool MatchPatternASCII(const std::string& eval, const std::string& pattern) {
	O3D_ASSERT(IsStringASCII(eval) && IsStringASCII(pattern));
...
size_t o3d::base::strlcpy(char* dst, const char* src, size_t dst_size) {
