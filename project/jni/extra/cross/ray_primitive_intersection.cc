...
o3d_extra::RayPrimitiveIntersectionFunctor::RayPrimitiveIntersectionFunctor
(const o3d::Point3& origin, const o3d::Vector3& direction)
: origin(origin)
, direction(direction)
...
void o3d_extra::RayPrimitiveIntersectionFunctor::ProcessTriangle
(
...
)
{
	...
	const o3d::Vector3 edge1(p1-p0);
	const o3d::Vector3 edge2(p2-p0);
	const o3d::Vector3 pvec(cross(direction, edge2));
	const o3d::Vector3 tvec(origin-p0);
	const o3d::Vector3 qvec(cross(tvec, edge1));
	const float det(dot(edge1, pvec));
	const float inv_det(1.f/det);
	const o3d::Vector3 tuv(
		o3d::Vector3(
			dot(edge2, qvec),
			dot(tvec, pvec),
			dot(direction, qvec)
