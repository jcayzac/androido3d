...
bool DrawPassInfo::Initialize(
  ...
  draw_list_ =
      DrawList::Ref(draw_list ? draw_list : pack->Create<DrawList>());

  state_set_ = StateSet::Ref(pack->Create<StateSet>());
  state_ = State::Ref(pack->Create<State>());
  state_set_->set_state(state_);
  state_set_->SetParent(parent);

  draw_pass_ = DrawPass::Ref(pack->Create<DrawPass>());
  draw_pass_->set_draw_list(draw_list);
  draw_pass_->set_sort_method(sort_method);
  draw_pass_->SetParent(state_set_);

  pack_ = Pack::Ref(pack);
  root_ = RenderNode::Ref(state_set_);
};
...
bool ViewInfo::Initialize(
  ...
  // Create Viewport
  viewport_ = Viewport::Ref(pack->Create<Viewport>());
  if (viewport) {
    viewport_->set_viewport(*viewport);
  }

  // Create a clear buffer.
  clear_buffer_ = ClearBuffer::Ref(pack->Create<ClearBuffer>());
  ...
  tree_traversal_ = TreeTraversal::Ref(pack->Create<TreeTraversal>());
  tree_traversal_->set_priority(priority++);
  tree_traversal_->SetParent(viewport_);
  tree_traversal_->set_transform(tree_root);

  pack_ = Pack::Ref(pack);
  render_graph_root_ = RenderNode::Ref(parent);
  tree_root_ = Transform::Ref(tree_root);
  root_ = RenderNode::Ref(viewport_);
  draw_context_ = DrawContext::Ref(
      draw_context ? draw_context :
                     DrawContext::Ref(pack->Create<DrawContext>()));
  priority_ = priority;
